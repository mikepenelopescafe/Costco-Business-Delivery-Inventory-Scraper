              ...
              if (enteredValue === this.zipCode) {
                ...
                if (enterKeyResult.success || enterKeyResult.hasTargetZip) {
                  ...
                  const modalSubmit = await this.page.evaluate(() => {
                    ...
                    // First, try to find the specific "Set Delivery ZIP Code" button
                    for (const button of buttons) {
                      const text = button.textContent || button.value || '';
                      const style = window.getComputedStyle(button);
                      const isVisible = style.display !== 'none' && style.visibility !== 'hidden';
                    
                    if (isVisible && text.toLowerCase().includes('set delivery zip')) {
                      button.click();
                      return { clicked: true, text: text.trim(), specific: true };
                    }
                  }
                  
                  // Fallback to other submit buttons that might work
                  for (const button of buttons) {
                    const text = button.textContent || button.value || '';
                    const style = window.getComputedStyle(button);
                    const isVisible = style.display !== 'none' && style.visibility !== 'hidden';
                    
                    if (isVisible && (text.toLowerCase().includes('set') || 
                                     text.toLowerCase().includes('save') || 
                                     text.toLowerCase().includes('update'))) {
                      button.click();
                      return { clicked: true, text: text.trim(), specific: false };
                    }
                  }

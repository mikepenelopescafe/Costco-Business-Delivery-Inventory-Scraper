...
class CostcoScraper {
  ...
  async setLocation() {
    ...
    try {
      // First, try direct navigation with ZIP code parameter
      logger.info('Attempting to set location via URL parameter...');
      await this.page.goto(`https://www.costcobusinessdelivery.com/?zipCode=${this.zipCode}`, { 
        waitUntil: 'networkidle2',
        timeout: 30000 
      });
      
      // Wait for page to process the ZIP parameter
      await new Promise(resolve => setTimeout(resolve, 3000));
      
      // Check if ZIP was set via URL
      const urlLocationSet = await this.page.evaluate((targetZip) => {
        const bodyText = document.body.innerText;
        const hasZip = bodyText.includes(`Delivery ZIP Code: ${targetZip}`) || 
                       bodyText.includes(`${targetZip}`);
        // Also check for the ZIP in any delivery-related text
        const deliveryElements = document.querySelectorAll('[class*="delivery"], [id*="delivery"]');
        for (const el of deliveryElements) {
          if (el.textContent && el.textContent.includes(targetZip)) {
            return true;
          }
        }
        return hasZip;
      ...
      if (urlLocationSet) {
        logger.info('✅ Successfully set location via URL parameter!');
        return; // Exit early if successful
      }
      
      logger.info('URL parameter did not set location, proceeding with UI method...');
      ...
      try {
        ...
        if (changeLocationLink.found) {
          ...
          try {
            ...
            try {
              // Wait for the modal dialog container
              await this.page.waitForSelector('.modal-dialog, .ui-dialog, [role="dialog"]', { 
                visible: true, 
                timeout: 10000 
              });
              logger.info('Modal dialog appeared');
              
              // Additional wait for modal animation/loading
              await new Promise(resolve => setTimeout(resolve, 2000));
              
              // Wait specifically for the ZIP input field to be visible
              await this.page.waitForFunction(() => {
                const input = document.querySelector('#WC_BusinessDeliveryBrowseForm_FormInput_zipCodeFormDeliveryZipCode');
              ...
              logger.info('ZIP input field is now visible');
            } catch (waitError) {
              logger.warn('Timeout waiting for modal/input visibility:', waitError.message);
              // Continue anyway in case the element is accessible
            }
            ...
            if (specificZipInput) {
              ...
              if (!isVisible) {
                ...
                const isVisibleNow = await specificZipInput.evaluate(el => {
                  const style = window.getComputedStyle(el);
                  return el.offsetParent !== null && 
                         style.display !== 'none' && 
                         style.visibility !== 'hidden';
                ...
                if (!isVisibleNow) {
                  logger.error('ZIP input still not visible after waiting');
                  
                  // Try to make it visible by interacting with the form
                  await this.page.evaluate(() => {
                    const input = document.querySelector('#WC_BusinessDeliveryBrowseForm_FormInput_zipCodeFormDeliveryZipCode');
                    if (input) {
                      // Try to focus parent form or container
                ...
                if (enterKeyResult.success || enterKeyResult.hasTargetZip) {
                  ...
                  const modalSubmit = await this.page.evaluate(() => {
                    ...
                    for (const button of buttons) {
                      ...
                      // Check both text content and value attribute for various button texts
                      const textLower = text.toLowerCase();
                      if (isVisible && inViewport && 
                          (textLower.includes('change delivery zip') ||
                           textLower.includes('set delivery zip') || 
                           (button.value && button.value.toLowerCase().includes('delivery zip')))) {
                        button.click();
                    ...
                    for (const input of submitInputs) {
                      ...
                      if (isVisible && (valueLower.includes('change delivery zip') || 
                                       valueLower.includes('set delivery zip') ||
                                       valueLower.includes('delivery zip'))) {
                        input.click();
                        return { clicked: true, text: value, specific: true, debugInfo: allButtons };
                      }
                    }
                    
                    // Final fallback: look for any primary/submit button in the modal
                    const modalContainer = document.querySelector('.modal-dialog, .ui-dialog, [role="dialog"]');
                    if (modalContainer) {
                      const modalButtons = modalContainer.querySelectorAll('button, input[type="submit"], input[type="button"]');
                      for (const button of modalButtons) {
                        ...
                        // Click any primary-looking button that's not Cancel
                        if (isVisible && 
                            ...
                            (classList.includes('primary') || 
                             ...
                             text.length > 0)) {
                          button.click();
                          return { clicked: true, text: text.trim(), fallback: true, debugInfo: allButtons };
                        }
                      }
                    }
